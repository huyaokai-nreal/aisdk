...
TEST_CASE("minimal session test") {
    auto interpreter = std::unique_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile("liftnimble_res26sw_250316_reproj_ebeff3.mnn"));
    CHECK(interpreter != nullptr);
    
    MNN::ScheduleConfig config;
    config.type = MNN_FORWARD_CPU;
    
    auto session = interpreter->createSession(config);  // 测试是否在此崩溃
    CHECK(session != nullptr);
}

TEST_CASE("testing lifnet mnn model") {
    //打开日志
    aisdk::base::Logger::GetInstance()->SetLogAllLevel(true);
    AISDK_LOG_INFO("start log recording");
    ...
    // 初始化MNN
    AISDK_LOG_INFO("begin to init mnn");
    MNN::ScheduleConfig config;
    ...
    // 加载模型
    AISDK_LOG_INFO("begin to load model");
    ...
    AISDK_LOG_INFO("begin to create session");
    auto session = interpreter->createSession(config);
    
    // 获取输入输出张量信息
    AISDK_LOG_INFO("begin to get session input");
    ...
    // 处理每个样本
    AISDK_LOG_INFO("going to process feat and mem file");
    for (int sample_id = 0; sample_id < 1; ++sample_id) {
        ...
        if (stat(sample_output_dir.c_str(), &st) != 0) {
            // 目录不存在，尝试创建
            AISDK_LOG_INFO("mkdir start: {}", sample_output_dir);
            int ret = mkdir(sample_output_dir.c_str(), 0755);
            if (ret != 0) {
                AISDK_LOG_INFO("mkdir {} failed: {}", sample_output_dir, strerror(errno));
            }
            CHECK(0 == ret);
        }
