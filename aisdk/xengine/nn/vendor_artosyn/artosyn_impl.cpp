...
ARTOSYN_AIModel::ARTOSYN_AIModel(ModelConfig &config) : AIModel() {
    ...
    s32Ret = AR_MPI_NPU_MallocCachedBuff((AR_CHAR *)"det-output", &stNPUOutBuff);
    if (s32Ret) {
        printf("AR_MPI_NPU_MallocCachedBuff failure!!");
        AR_MPI_NPU_FreeBuff(&stNPUInBuff);
    }
    ...
    printf(
        ...
        stInTensor.u32ID, stInTensor.u32Bank, stInTensor.u32Offset, stInTensor.u32Height, stInTensor.u32KStep,
        stInTensor.u32KNormNum, stInTensor.u32KSizeLast, stInTensor.u32KSizeNorm, stInTensor.achName,
        stInTensor.achType, stInTensor.u32Num, stInTensor.u32OriChannels, stInTensor.u32OriFrameSize,
        stInTensor.u32Precision, stInTensor.u32RowStep, stInTensor.u32TensorStep, (float)stInTensor.dScaleFactor,
        stInTensor.u32Size, stInTensor.u32Width, stInTensor.s32ZeroPoint, stInTensor.achLayoutType);
    ...
    printf(
        ...
        "[4]%d,%d,%d,%d [5]%f,%d,%d,%d [6]%s,\n",
        stOutTensor.u32ID, stOutTensor.u32Bank, stOutTensor.u32Offset, stOutTensor.u32Height, stOutTensor.u32KStep,
        stOutTensor.u32KNormNum, stOutTensor.u32KSizeLast, stOutTensor.u32KSizeNorm, stOutTensor.achName,
        stOutTensor.achType, stOutTensor.u32Num, stOutTensor.u32OriChannels, stOutTensor.u32OriFrameSize,
        stOutTensor.u32Precision, stOutTensor.u32RowStep, stOutTensor.u32TensorStep, (float)stOutTensor.dScaleFactor,
        stOutTensor.u32Size, stOutTensor.u32Width, stOutTensor.s32ZeroPoint, stOutTensor.achLayoutType);
}
...
Status ARTOSYN_Session::Init(std::shared_ptr<AIModel> &model, SessionConfig &Sconfig) { return Status::SUCCESS; }
