...
INCBIN(mano_left_model, MANO_LEFT_MODEL_PATH);
INCBIN(mano_right_model, MANO_RIGHT_MODEL_PATH);

void decodeAESKey(std::string hexcode, unsigned char* keybuf) {
	int               i = 0;
	std::stringstream ss;
	ss << hexcode;
	std::string p;
...
// --------------------------------------
std::vector<int>   inds            = {0, 13, 14, 15, 16, 1, 2, 3, 17, 4, 5, 6, 18, 10, 11, 12, 19, 7, 8, 9, 20};
std::map<int, int> vert_inds_left  = {{16, 745}, {17, 317}, {18, 445}, {19, 556}, {20, 673}};
std::map<int, int> vert_inds_right = {{16, 745}, {17, 317}, {18, 444}, {19, 556}, {20, 673}};
// --------------------------------------
...
void print1dArray(cnpy::NpyArray& arr) {
	std::cout << std::fixed << std::setprecision(4) << std::setfill(' ');
	for (size_t i = 0; i < arr.shape[0]; i++) {
		std::cout << std::setw(8) << arr.data<float>()[i] << " ";
	}
	std::cout << " ================= " << std::endl;
}
// --------------------------------------
...
//!
//! \brief Convert rotvec to rotmat, please check "Rodrigues' rotation formula"
//! @param axisang {float*}: rotvec ptr, 1x3
//! @param rot_mat {float*}: rotmat ptr, 3x3
//!
void rodrigues(float* axisang, float* rot_mat) {
	...
	float quat[4] = {v_cos, v_sin * axisang_normalized[0], v_sin * axisang_normalized[1],
					 v_sin * axisang_normalized[2]};
...
//!
//! \brief Compute pose_map and rot_mat from rot_vecs
//! @param pose_vectors {std::vector<float>}: rot_vecs
//! @param pose_maps {cnpy::NpyArray}: pose_map
//! @param rot_mats {cnpy::NpyArray}: rot_mat
//!
void posemap_axisang(std::vector<float>& pose_vectors, cnpy::NpyArray& pose_maps, cnpy::NpyArray& rot_mats) {
	...
	for (int i = 0; i < rot_nb; ++i) {
		float  rot_mat[9];
		size_t ind_vec = i * 3;
		float  v[3]    = {pose_vectors[i * 3], pose_vectors[i * 3 + 1], pose_vectors[i * 3 + 2]};
		// std::cout << "wocao: " << v[0] << " " << v[1] << " " << v[2] <<
		// std::endl;
		rodrigues(v, rot_mat);
		for (int j = 0; j < 3; ++j) {
			...
			for (int k = 0; k < 3; ++k) {
				...
				// first mat(root) should not do it!
				if (j == k && i > 0) {
...
//!
//! \brief Compute pose_map and rot_mat() from rot_mat
//! @param rot_mat_vec {std::vector<float>}: rot_vecs
//! @param pose_maps {cnpy::NpyArray}: pose_map
//! @param rot_mats {cnpy::NpyArray}: rot_mat
//!
void posemap_axisang(std::vector<Eigen::Matrix3f>& rot_mat_vec, cnpy::NpyArray& pose_maps, cnpy::NpyArray& rot_mats) {
	// number of vectors to convert
	size_t rot_nb = int(rot_mat_vec.size());
	// shape of the result tensors
	std::vector<size_t> result_sz = {rot_nb, 3, 3};
	// result tensors
	pose_maps = cnpy::NpyArray(result_sz, sizeof(float), false);
	...
	// iterare vectors
	for (int i = 0; i < rot_nb; ++i) {
		Eigen::Matrix3f temp    = Eigen::Map<Eigen::Matrix<float, 3, 3, Eigen::RowMajor>>(rot_mat_vec[i].data());
		float*          rot_mat = (float*)(temp.data());
		for (int j = 0; j < 3; ++j) {
			// #pragma omp parallel for
			for (int k = 0; k < 3; ++k) {
				size_t ind4x4                  = i * 9 + j * 3 + k;
...
//!
//! \brief ctor of Skeleton
//!
smpl::Skeleton::Skeleton() { joints_21_resvec.resize(21 * 3); }
//!
//! \brief Clone NpyArray to another
//! @param src {cnpy::NpyArray}: src npyarray struct
//! @param dst {cnpy::NpyArray}: dst npyarray struct
//!
...
//!
//! \brief Fixed 4x4 matmul func
//! @param A {float*}: left matrix ptr, fixed size 4x4
//! @param B {float*}: right matrix ptr, fixed size 4x4
//! @param dst {float*}: result matrix ptr, fixed size 4x4
//!
...
//
//--------------------------------------------------------------------------
void smpl::BlendTransforms(cnpy::NpyArray& weights, cnpy::NpyArray& joint_rel_transforms,
...
//
//--------------------------------------------------------------------------
void smpl::LinearBlend(cnpy::NpyArray& v_shaped, cnpy::NpyArray& weights, cnpy::NpyArray& joint_rel_transforms,
...
void smpl::Skeleton::parse(cnpy::NpyArray& Kinematic_tree) {
	...
	std::vector<int> nodes(N_nodes, 0);  // ������� �����
...
void smpl::Skeleton::setPose(std::vector<float>& joints_rots_vector) {
	...
	//         {
	//             this->joints_rots_vector[i] += hands_mean.data<float>()[i -
	//             3];
	//         }
	//     }
	// relative rotation of joints, same as joints_rots_vector, but in matrix
	// form
	...
	template_joints_rel_coords = cnpy::NpyArray(template_joints_abs_coords.shape, template_joints_abs_coords.word_size,
												template_joints_abs_coords.fortran_order);
...
#pragma omp parallel for
		for (int j = 1; j < chains[i].size(); j++) {
			...
			template_joints_rel_coords.data<float>()[joint_ind2 * 3 + 0] =
				template_joints_abs_coords.data<float>()[joint_ind2 * 3 + 0] -
				template_joints_abs_coords.data<float>()[joint_ind1 * 3 + 0];
			template_joints_rel_coords.data<float>()[joint_ind2 * 3 + 1] =
				template_joints_abs_coords.data<float>()[joint_ind2 * 3 + 1] -
				template_joints_abs_coords.data<float>()[joint_ind1 * 3 + 1];
			template_joints_rel_coords.data<float>()[joint_ind2 * 3 + 2] =
				template_joints_abs_coords.data<float>()[joint_ind2 * 3 + 2] -
				template_joints_abs_coords.data<float>()[joint_ind1 * 3 + 2];
		}
	}
	// std::cout << "template_joints_relative_rotations??" << std::endl;
	// print3dArray(template_joints_relative_rotations);
	// memset(template_joints_rel_coords.data<float>(), 0, sizeof(float) * 3);
	// compute relative transforms of joints1
	// std::cout << "template_joints_rel_coords" << std::endl;
	// print2dArray(template_joints_rel_coords);
	makeTrasformsMatrix(template_joints_relative_rotations, template_joints_rel_coords, joint_rel_transforms);
	// python -> rel_transform_flt
	// std::cout << "joint_rel_transforms" << std::endl;
	// print3dArray(joint_rel_transforms);
...
// Set skeleton pose
//-----------------------------------
void smpl::Skeleton::setPose(std::vector<Eigen::Matrix3f> joints_rots_mat) {
	// relative rotation of joints, same as joints_rots_vector, but in matrix
	// form
	cnpy::NpyArray template_joints_relative_rotations;
	// compute rotation matrices
	posemap_axisang(joints_rots_mat, pose_maps, template_joints_relative_rotations);
...
void smpl::loadModel(std::string filename) {
	...
	if (this->handmode == 0) {
		unsigned char* decryptFileBuf = new unsigned char[gmano_left_model_size];

		unsigned char rand_key[AES_KEYLEN];

		std::string hexcode = MANO_LEFT_MODEL_KEY;

		decodeAESKey(hexcode, rand_key);

		AES.setKey(rand_key, XrealAITools::aes::AESBIT::AES_256);

		int decryptfilesize =
			AES.decryptBuf(const_cast<unsigned char*>(gmano_left_model_data), gmano_left_model_size, decryptFileBuf);

		npz_map = cnpy::npz_load(static_cast<void*>(decryptFileBuf), gmano_left_model_size);
	} else {
		unsigned char* decryptFileBuf = new unsigned char[gmano_right_model_size];

		unsigned char rand_key[AES_KEYLEN];

		std::string hexcode = MANO_RIGHT_MODEL_KEY;
		...
		int decryptfilesize =
			AES.decryptBuf(const_cast<unsigned char*>(gmano_right_model_data), gmano_right_model_size, decryptFileBuf);

		npz_map = cnpy::npz_load(static_cast<void*>(decryptFileBuf), gmano_right_model_size);
	}
	// kinematicTree
	kinematicTree = npz_map["kintree_table"];  // 2,n_joints
	// std::cout << "kinematicTree" << std::endl;
	// std::cout << "kinematicTree shape=" << "[" << kinematicTree.shape[0] <<
	// "," << kinematicTree.shape[1] << "]" << std::endl; std::cout <<
	// "-----------------" << std::endl;
	...
	// std::cout << std::endl;
	vetexNormals =
		cnpy::NpyArray(vertices_template.shape, vertices_template.word_size, vertices_template.fortran_order);
	auto faceInd = faceIndices.as_vec<unsigned int>();
	// normalsComputer = new NormalsComputer(faceInd);
	// std::cout << "---------------------------" << std::endl;
...
void smpl::updateMesh(std::vector<float>& shape, std::vector<float>& pose) {
	...
	std::chrono::steady_clock::time_point end_setp = std::chrono::steady_clock::now();
	...
	// std::cout << "joint_abs_transforms" << std::endl;
	// // printDims(v_posed);
	// print2dArray(posed_vertices);
	auto joint_abs_transforms_ptr = skeleton.joint_abs_transforms.data<float>();
	auto posed_vertices_ptr       = posed_vertices.data<float>();
	// normalsComputer->getVertexNormals(vertex_num * 3,
	// posed_vertices.data<float>(), vetexNormals.data<float>()); vert_inds_left
	for (int i = 0; i < 21; i++) {
		if (inds[i] < 16) {
			skeleton.joints_21_resvec[i * 3]     = joint_abs_transforms_ptr[inds[i] * 16 + 3];
			skeleton.joints_21_resvec[i * 3 + 1] = joint_abs_transforms_ptr[inds[i] * 16 + 7];
			skeleton.joints_21_resvec[i * 3 + 2] = joint_abs_transforms_ptr[inds[i] * 16 + 11];
		} else {
			skeleton.joints_21_resvec[i * 3]     = posed_vertices_ptr[vert_inds_right[inds[i]] * 3];
			skeleton.joints_21_resvec[i * 3 + 1] = posed_vertices_ptr[vert_inds_right[inds[i]] * 3 + 1];
			skeleton.joints_21_resvec[i * 3 + 2] = posed_vertices_ptr[vert_inds_right[inds[i]] * 3 + 2];
		}
	}
...
//
//--------------------------------------------------------------------------
void smpl::updateMesh(std::vector<float>& shape, std::vector<Eigen::Matrix3f> pose_mat) {
	// std::chrono::steady_clock::time_point begin =
	// std::chrono::steady_clock::now();

	assert(joint_num == pose_mat.size());
	// ----------------------------------
	// Compute initial shape and skeleton
	// ----------------------------------
	float* beta = nullptr;

	for (auto mat : pose_mat) {
		Eigen::JacobiSVD<Eigen::MatrixXf> svd(mat, Eigen::ComputeFullV | Eigen::ComputeFullU);
		...
		// std::cout << "pR_matrix = " << mat << std::endl;
	}
	...
	// normalsComputer->getVertexNormals(vertex_num * 3,
	// posed_vertices.data<float>(), vetexNormals.data<float>()); vert_inds_left
	auto vert_inds = handmode ? vert_inds_right : vert_inds_left;
	// HANDTRACKING_LOG_TRACE("hand_current_data_-1:===============================")
	// for (int k = 0; k < 16; k++) {
	// 	HANDTRACKING_LOG_TRACE("hand_current_data_-1: ind:%d, \n %f %f %f \n %f %f %f \n %f %f %f", k,
	// 						   pose_mat[k](0, 0), pose_mat[k](0, 1), pose_mat[k](0, 2), pose_mat[k](1, 0),
	// 						   pose_mat[k](1, 1), pose_mat[k](1, 2), pose_mat[k](2, 0), pose_mat[k](2, 1),
	// 						   pose_mat[k](2, 2));
	// }
	for (int i = 0; i < 21; i++) {
		if (inds[i] < 16) {
			...
			skeleton.joints_21_resvec[i * 3]     = posed_vertices_ptr[vert_inds[inds[i]] * 3];
			skeleton.joints_21_resvec[i * 3 + 1] = posed_vertices_ptr[vert_inds[inds[i]] * 3 + 1];
			skeleton.joints_21_resvec[i * 3 + 2] = posed_vertices_ptr[vert_inds[inds[i]] * 3 + 2];
		}

		// HANDTRACKING_LOG_TRACE("hand_current_data_-1: ind:%d, %f %f %f", inds[i], skeleton.joints_21_resvec[i * 3],
		// 					   skeleton.joints_21_resvec[i * 3 + 1], skeleton.joints_21_resvec[i * 3 + 2]);
	}
...
//--------------------------------------------------------------------------
void smpl::computeBlendShape(cnpy::NpyArray& Beta, cnpy::NpyArray& MeanShape, cnpy::NpyArray& ShapeBlendBasis,
...
//--------------------------------------------------------------------------
void smpl::computeBlendPose(cnpy::NpyArray& PoseMap, cnpy::NpyArray& V_Shaped, cnpy::NpyArray& PoseBlendBasis,
