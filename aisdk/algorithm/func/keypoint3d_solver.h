...
class Keypoint3DSolver {
   public:
    ...
    [[nodiscard]] absl::StatusOr<Eigen::Matrix<float, 21, 3>> SolveKeypoints(const Eigen::Matrix<float, 21, 3>& kpt25d,
                                                              ...
                                                              bool flip_x_axis, bool source_change=false) const;

   private:
    struct CostFunctor {
        CostFunctor(Eigen::Matrix<float, 25, 3>  norm_kpt3d, Eigen::Matrix<float, 25, 1>  rel_depth,
                    Eigen::Matrix<float, 20, 1>  bones, Eigen::Matrix<float, 25, 3> last_kpt3d, float last_kpt3d_weight)
        ...
        bool operator()(const T* const _root_depth, T* residual) const {
            ...
            Eigen::Matrix<T, 25, 3> last_kpt3d = last_kpt3d_.cast<T>();
            kpt3d = norm_kpt3d.array().colwise() * (root_depth + rel_depth.array().col(0));
            Eigen::Matrix<T, 20, 1> e_bones = Eigen::Matrix<T, 20, 1>::Ones();
            Eigen::Map<Eigen::Matrix<T, 20, 1>> result_bones(residual);
            Eigen::Map<Eigen::Matrix<T, 1, 3>> result_kpt(residual+20);
            ...
            result_bones = e_bones - bones;
            result_kpt = (last_kpt3d.row(0) - kpt3d.row(0)).array() * T(last_kpt3d_weight_);
