...
class smpl {
	...
	class NormalsComputer {
	   ...
	   public:
		...
		// input: vertices as array of floats in v1x,v1y,v1z,...vmx,vmy,vmz
		// format
		//--------------------------------------------------------------------------
	   ...
	   private:
		...
		//--------------------------------------------------------------------------
		void computeFaceNormals(std::vector<unsigned int>& faces, std::vector<float>& vertices,
		...
		//--------------------------------------------------------------------------
		void computeVertexNormals(std::vector<unsigned int>& faces, std::vector<float>& vertices,
			...
			for (unsigned int i = 0; i < sharedTriangles.size(); ++i) {
				...
				float n = sqrt(result[i * 3 + 0] * result[i * 3 + 0] + result[i * 3 + 1] * result[i * 3 + 1] +
		...
		//--------------------------------------------------------------------------
		void computeVertexNormals(unsigned int nFaces, unsigned int nVertices, unsigned int* faces, float* vertices,
								  float* result) {
	...
	class Skeleton {
	   public:
		...
		std::vector<float> joints_21_resvec;
		...
		void setPose(std::vector<Eigen::Matrix3f> joints_rots_mat);
	};
   ...
   public:
	//!
	//! \brief Load MANO model with npz format
	//! @param filename: MANO model path
	//!
	void loadModel(std::string filename);
	//--------------------------------------------------------------------------
	//
	//--------------------------------------------------------------------------
	void computeBlendShape(cnpy::NpyArray& Beta, cnpy::NpyArray& MeanShape, cnpy::NpyArray& ShapeBlendBasis,
						   cnpy::NpyArray& V_Shaped);
	//--------------------------------------------------------------------------
	//
	//--------------------------------------------------------------------------

	void computeBlendPose(cnpy::NpyArray& PoseMap, cnpy::NpyArray& V_Shaped, cnpy::NpyArray& PoseBlendBasis,
						  cnpy::NpyArray& V_Posed);
	//--------------------------------------------------------------------------
	//
	//--------------------------------------------------------------------------
	void BlendTransforms(cnpy::NpyArray& weights, cnpy::NpyArray& joint_rel_transforms,
						 cnpy::NpyArray& blendedTransforms);
	//--------------------------------------------------------------------------
	//
	//--------------------------------------------------------------------------
	void LinearBlend(cnpy::NpyArray& template_vertices, cnpy::NpyArray& weights, cnpy::NpyArray& joint_rel_transforms,
					 cnpy::NpyArray& posed_vertices);
	//--------------------------------------------------------------------------
	//
	//--------------------------------------------------------------------------
	...
	//
	//--------------------------------------------------------------------------
	void updateMesh(std::vector<float>& shape, std::vector<Eigen::Matrix3f> pose_mat);
};
//--------------------------------------------------------------------------
//
//--------------------------------------------------------------------------
