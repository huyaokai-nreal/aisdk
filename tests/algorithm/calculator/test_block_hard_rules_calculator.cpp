...
TEST_CASE("testing block hard rule calculator in graph") {
    ...
    status = graph.AddPacketToInputStream("in", mediapipe::MakePacket<Kpt3dData>(*input).At(mediapipe::Timestamp(1)));
    CHECK(status.ok());
...
TEST_CASE("test block hard rule calculator") {
    ...
    input->lhand_valid = true;
    input->rhand_valid = true;
    runner.MutableInputs()->Index(0).packets.push_back(xgraph::Adopt(input.release()).At(xgraph::Timestamp::PostStream()));
