...
const mediapipe::LazyRE2 kValidColumnRegex = {"^[a-zA-Z0-9_?*]+$"};
const mediapipe::LazyRE2 kReplace1WildcharRegex = {"\\?"};
const mediapipe::LazyRE2 kReplace0toNWildcharRegex = {"\\*"};
...
absl::btree_map<std::string,
    ...
    kColumns = {
        ...
        {"time_max",
         [](const CalculatorData& d) -> const std::string {
           return ToStringF(d.time_stat.max());
         }},
        {"time_95",
        [](const CalculatorData& d) -> const std::string {
          return ToStringF(d.time_stat.percentile_95());
        }},
        {"time_total",
         [](const CalculatorData& d) -> const std::string {
        ...
        {"input_latency_max",
         [](const CalculatorData& d) -> const std::string {
           return ToStringF(d.input_latency_stat.max());
         }},
        {"input_latency_95",
         [](const CalculatorData& d) -> const std::string {
           return ToStringF(d.input_latency_stat.percentile_95());
         }},
        {"input_latency_total",
         [](const CalculatorData& d) -> const std::string {
...
absl::Status Reporter::set_columns(const std::vector<std::string>& columns) {
  ...
  for (const auto& column_matcher : columns) {
    if (!mediapipe::RE2::PartialMatch(column_matcher, *kValidColumnRegex)) {
      warnings << "Column '" << column_matcher << "' is invalid." << std::endl;
      error = true;
      continue;
    }
    std::string colString = column_matcher;
    mediapipe::RE2::GlobalReplace(&colString, *kReplace0toNWildcharRegex, ".*");
    mediapipe::RE2::GlobalReplace(&colString, *kReplace1WildcharRegex, ".");
    ...
    for (const auto& column : kColumns) {
      if (mediapipe::RE2::FullMatch(column.first, colString)) {
        matched = true;
