...
static const unsigned int READ_BUF_LEN = 100 * 1024 * 1024;
#if AES_BIT == 256
unsigned char gAESKEY[AES_KEYLEN] = {0x45, 0x4d, 0x5e, 0xdf, 0x8c, 0xae, 0x53, 0x23, 0xc4, 0xed, 0xac,
                                     0x99, 0xaa, 0x78, 0x29, 0x45, 0x11, 0x54, 0x36, 0x65, 0x45, 0x88,
                                     0xac, 0xea, 0xde, 0xdd, 0xea, 0xeb, 0xbd, 0x8b, 0x9a, 0xb5};
#elif AES_BIT == 192
unsigned char gAESKEY[AES_KEYLEN] = {0x85, 0x4d, 0x55, 0xdf, 0x8c, 0xae, 0x53, 0x23, 0xca, 0xed, 0xac, 0x99,
...
int aes::encryptBuf(unsigned char *inbuf, int inbuflen, unsigned char *outbuf) {
    ...
    for (int i = block_remain; i < 16; i++) mInbuf[i + block * 16] = remain;
    AES_encrypt(&mInbuf[block * 16], &outbuf[block * 16], &aeskey);
    block++;
    for (int i = 0; i < 16; i++) mInbuf[i + block * 16] = remain;
    AES_encrypt(&mInbuf[block * 16], &outbuf[block * 16], &aeskey);
    block++;
...
int aes::decryptBuf(unsigned char *inbuf, int inbuflen, unsigned char *outbuf) {
    ...
    for (int i = 0; i < block; i++) AES_decrypt(&inbuf[i * 16], &outbuf[i * 16], &aeskey);
...
int aes::encryptFile(const char *src_path, const char *dst_path) {
    ...
    while (read_file_size > 0) {
        ...
        if (block_remain != 0) {
            remain = 32 - block_remain;
            for (int i = block_remain; i < 16; i++) inbuf[i + block * 16] = remain;
            AES_encrypt(&inbuf[block * 16], &outbuf[block * 16], &aeskey);
            block++;
            for (int i = 0; i < 16; i++) inbuf[i + block * 16] = remain;
            AES_encrypt(&inbuf[block * 16], &outbuf[block * 16], &aeskey);
            block++;
        }
