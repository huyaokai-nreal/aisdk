...
def read_parser():
    ...
    parser.add_argument('--device', '-d', type=str, default='10.2.16.130',
                        help='选择设备的ip地址')
    parser.add_argument('--request', '-r', type=str, required= True, choices=["prepare","batchreport","deletecache"],
                        help='选择动作')
...
if __name__ == "__main__":  
    ...
    if request == "prepare":
        ...
        run_cmd('adb -s {} wait-for-device push {} {} {} {}'.format(device, config_path, sdk_json_path, libnr_hand_algo_so_path, files_path))
        ...
        #     adb_pull_command = ['adb', '-s', device, 'wait-for-device', 'pull', record_path, './']
        ...
        #     # 假设脚本在当前目录下
        #     result = subprocess.run(["bash", "./build_hand_res.sh", record_name, fps])
